...
/* ---------- welcome ---------- */
.tcp-welcome-emoji { font-size: 52px; margin-bottom: 6px; animation: tcp-bob 2.4s ease-in-out infinite alternate; }
.tcp-welcome h2 { font-family: 'Fredoka', sans-serif; font-weight: 700; font-size: 26px; color: var(--pier-bulb); margin: 4px 0 10px; }
.tcp-welcome-blurb { font-size: 15px; line-height: 1.4; color: rgba(246, 235, 212, .82); margin: 0 0 16px; }
.tcp-nana-line {
...
.tcp-nana-avatar { font-size: 24px; flex: 0 0 auto; }
.tcp-start { min-height: 60px; padding: 0 34px; font-size: 18px; }
@keyframes tcp-bob { from { transform: translateY(0); } to { transform: translateY(-6px); } }

/* ---------- table picker ---------- */
.tcp-picker h2 { font-family: 'Fredoka', sans-serif; font-weight: 700; font-size: 24px; color: var(--pier-bulb); margin: 0 0 4px; }
.tcp-picker-sub { font-size: 14px; color: rgba(246, 235, 212, .75); margin: 0 0 18px; }
.tcp-cupgrid { display: grid; grid-template-columns: repeat(auto-fit, minmax(76px, 1fr)); gap: 12px; }
.tcp-cup {
...
.tcp-cup-emoji { font-size: 26px; }
.tcp-cup-num { font-family: 'Fredoka', sans-serif; font-weight: 700; font-size: 19px; color: var(--pier-teal); }

/* ---------- lap change banner ---------- */
.tcp-lapcard-emoji { font-size: 48px; margin-bottom: 8px; }
.tcp-lapcard h2 { font-family: 'Fredoka', sans-serif; font-weight: 700; font-size: 23px; color: var(--pier-bulb); margin: 2px 0 10px; }
.tcp-lapcard p { font-size: 15px; line-height: 1.4; color: rgba(246, 235, 212, .85); margin: 0 0 20px; }
.tcp-lapcard p b { color: var(--pier-pink); }
.tcp-lap-go { min-height: 60px; padding: 0 28px; font-size: 16px; }

/* ---------- lap play ---------- */
.tcp-lap { display: flex; flex-direction: column; align-items: center; gap: 14px; }
.tcp-lap-header { width: 100%; display: flex; flex-direction: column; align-items: center; gap: 8px; }
.tcp-lap-title { font-family: 'Fredoka', sans-serif; font-weight: 700; font-size: 17px; color: var(--pier-bulb); letter-spacing: .02em; }
.tcp-lap-dir { color: var(--pier-teal); margin-left: 8px; font-size: 14px; }
.tcp-dots { display: flex; flex-wrap: wrap; justify-content: center; gap: 6px; max-width: 280px; }
.tcp-dot { width: 10px; height: 10px; border-radius: 50%; background: rgba(255, 255, 255, .18); }
.tcp-dot.done { background: var(--correct); }
.tcp-dot.current { background: var(--pier-bulb); box-shadow: 0 0 0 3px rgba(255, 233, 168, .3); }
.tcp-cuparea { position: relative; display: flex; flex-direction: column; align-items: center; gap: 10px; padding-top: 6px; }
.tcp-cuparea.tcp-flash-correct::before {
...
.tcp-cup-big { font-size: 64px; filter: drop-shadow(0 6px 10px rgba(0, 0, 0, .45)); }
...
.tcp-cup-big.tcp-wobble { animation: tcp-wobble 420ms ease both; }
@keyframes tcp-spin-fwd { 0% { transform: rotate(0deg) scale(1); } 55% { transform: rotate(200deg) scale(.82); } 100% { transform: rotate(360deg) scale(1); } }
@keyframes tcp-spin-back { 0% { transform: rotate(0deg) scale(1); } 55% { transform: rotate(-200deg) scale(.82); } 100% { transform: rotate(-360deg) scale(1); } }
...
.tcp-stem { font-size: clamp(22px, 4.4vw, 30px); font-weight: 700; color: var(--parchment); text-align: center; min-height: 40px; }
.tcp-hint {
...
.tcp-hint.show { opacity: 1; padding: 10px 16px; max-height: 120px; }
.tcp-hint b { color: var(--pier-teal); }
.tcp-hint-tag { display: block; font-size: 10.5px; letter-spacing: .08em; font-weight: 700; color: var(--pier-pink); margin-bottom: 3px; }
.tcp-numhost { margin-top: 2px; }
...
.tcp-sticker-emoji { font-size: 24px; margin-bottom: 4px; }
.tcp-sticker-text { font-family: 'Fredoka', sans-serif; font-weight: 700; font-size: 19px; line-height: 1.15; }
.tcp-end-sub { font-size: 14.5px; line-height: 1.4; color: rgba(246, 235, 212, .85); margin: 0 0 20px; }
.tcp-ride-chip {
...
.tcp-end-btns { display: flex; flex-wrap: wrap; justify-content: center; gap: 12px; }
.tcp-end-btns .btn { min-height: 60px; padding: 0 24px; font-size: 15px; }
`;
...
export default {
  ...
  mount(host, ctx, pier) {
    ...
    const back = el('button', 'btn btn-ghost tcp-back', '← PIER');
    back.addEventListener('click', () => { ctx.audio.sfx('back'); ctx.go('#/pier'); });
    const stage = el('div', 'tcp-stage');
    ...
    function renderWelcome() {
      ...
      const card = el('div', 'tcp-card tcp-welcome enter-pop');
      card.innerHTML = `
        <div class="tcp-welcome-emoji">🍵</div>
        <h2>THE TEACUPS</h2>
        <p class="tcp-welcome-blurb">A gentle spin through one whole times table — no clock, no score, just you and the cups.</p>
    ...
    function renderPicker() {
      ...
      const wrap = el('div', 'tcp-card tcp-picker enter-pop');
      wrap.innerHTML = `
        <h2>PICK YOUR CUP</h2>
        <p class="tcp-picker-sub">${deluxe ? 'Deluxe is on — even 11 and 12 fancy a spin today!' : 'Which table fancies a spin today?'}</p>
      `;
    ...
    function renderLapBanner(onGo) {
      clearStage();
      const wrap = el('div', 'tcp-card tcp-lapcard enter-pop');
      wrap.innerHTML = lap === 1
        ? `<div class="tcp-lapcard-emoji">🍵</div><h2>TABLE ${table}</h2><p>Lap 1 — multiplication! Answer each cup as it spins round.</p>`
        : `<div class="tcp-lapcard-emoji">🔄</div><h2>LAP 1 DONE!</h2><p>Now the cups spin <b>BACKWARDS</b> — division time!</p>`;
      const goBtn = el('button', 'btn btn-gold tcp-lap-go', lap === 1 ? "LET'S GO! 🍵" : 'SPIN BACKWARDS! 🔄');
      wrap.append(goBtn);
      stage.append(wrap);
    ...
    function buildLapUI() {
      clearStage();
      busy = false;
      const wrap = el('div', 'tcp-card tcp-lap enter-pop');

      const header = el('div', 'tcp-lap-header');
      header.innerHTML = `<div class="tcp-lap-title">TABLE ${table}<span class="tcp-lap-dir">${lap === 1 ? '× MULTIPLY' : '÷ DIVIDE'}</span></div>`;
    ...
    function finish() {
      ...
      const wrap = el('div', 'tcp-card tcp-end enter-pop');
      wrap.innerHTML = `
        <div class="tcp-sticker">
          <div class="tcp-sticker-emoji">✨🍵✨</div>
          <div class="tcp-sticker-text">TABLE ${table}<br>POLISHED!</div>
        </div>
        <p class="tcp-end-sub">Both laps spun clean — multiplication AND the division inverses. Lovely, gentle work, nose-soldier.</p>
      `;
